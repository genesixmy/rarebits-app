-- SHIP-FIX-1:
-- 1) Single source of truth for delivery_required rule.
-- 2) mark_invoice_as_paid should only auto-create shipment when delivery_required = TRUE.

DROP FUNCTION IF EXISTS public.invoice_delivery_required(TEXT, NUMERIC);

CREATE OR REPLACE FUNCTION public.invoice_delivery_required(
...
AS $$
  WITH normalized AS (
    SELECT
      lower(regexp_replace(btrim(COALESCE(p_shipping_method, '')), '[\s-]+', '_', 'g')) AS shipping_method,
      GREATEST(COALESCE(p_shipping_charged, 0), 0) AS shipping_charged
  )
  SELECT
    (shipping_charged > 0)
    OR shipping_method IN ('courier', 'post', 'delivery', 'prepaid', 'cod')
  FROM normalized;
$$;

GRANT EXECUTE ON FUNCTION public.invoice_delivery_required(TEXT, NUMERIC) TO authenticated;
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.invoices
      DROP CONSTRAINT invoices_shipping_method_check;
  END IF;
  ...
  ALTER TABLE public.invoices
    ...
    CHECK (
      ...
      OR lower(btrim(shipping_method)) IN (
        'prepaid',
        'cod',
        'courier',
        'post',
