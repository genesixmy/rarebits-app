-- INV-ADJ-3:
-- Treat goodwill adjustment as invoice revenue adjustment (not expense source).

ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS adjustment_total NUMERIC,
  ADD COLUMN IF NOT EXISTS final_total NUMERIC;

UPDATE public.invoices
SET adjustment_total = GREATEST(COALESCE(adjustment_total, 0), 0)
WHERE adjustment_total IS NULL
   OR adjustment_total < 0;

UPDATE public.invoices
SET final_total = GREATEST(
  COALESCE(total_amount, 0) - GREATEST(COALESCE(adjustment_total, 0), 0),
  0
)
...
ALTER TABLE public.invoices
  ...
  ALTER COLUMN adjustment_total SET NOT NULL,
...
BEGIN
  ...
  NEW.final_total := ROUND(
    GREATEST(COALESCE(NEW.total_amount, 0) - NEW.adjustment_total, 0),
    2
  );
  RETURN NEW;
END;
...
BEFORE INSERT OR UPDATE OF total_amount, adjustment_total
ON public.invoices
FOR EACH ROW
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'invoices_adjustment_total_nonnegative_check'
      AND conrelid = 'public.invoices'::regclass
  ) THEN
    ALTER TABLE public.invoices
      ADD CONSTRAINT invoices_adjustment_total_nonnegative_check
      CHECK (adjustment_total >= 0);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'invoices_final_total_nonnegative_check'
      AND conrelid = 'public.invoices'::regclass
  ) THEN
    ALTER TABLE public.invoices
      ADD CONSTRAINT invoices_final_total_nonnegative_check
      CHECK (final_total >= 0);
  END IF;
END $$;

-- Reclassify legacy refund rows as adjustment (not expense).
UPDATE public.transactions
SET
  type = 'goodwill_adjustment',
  transaction_type = 'adjustment'
WHERE lower(COALESCE(type, '')) IN ('refund', 'refund_adjustment', 'goodwill_adjustment')
  AND (
    transaction_type IS NULL
    OR lower(COALESCE(transaction_type, '')) <> 'adjustment'
    OR lower(COALESCE(type, '')) IN ('refund', 'refund_adjustment')
  );
