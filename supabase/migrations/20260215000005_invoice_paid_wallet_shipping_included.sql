-- SHIP-2: Wallet integration on invoice paid (shipping included exactly once)
-- Wallet inflow on paid invoice must always use invoices.total_amount.
-- IMPORTANT:
-- total_amount already includes shipping_charged.
-- Never add shipping_charged again during wallet insertion.
-- Shipping cost deduction is handled separately by shipment expense flow.
...
SET shipping_state = CASE
  WHEN status = 'paid'
    AND (
      GREATEST(COALESCE(shipping_charged, 0), 0) > 0
      OR (
        COALESCE(shipping_required, FALSE) = TRUE
        AND lower(btrim(COALESCE(shipping_method, ''))) NOT IN ('pickup', 'meetup', 'selfpickup')
      )
    )
    THEN 'ready_to_ship'
  ELSE 'not_required'
...
WHERE shipping_state IS NULL
   OR btrim(shipping_state) = '';

ALTER TABLE public.invoices
  ALTER COLUMN shipping_state SET DEFAULT 'not_required';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'invoices_shipping_state_check'
      AND conrelid = 'public.invoices'::regclass
  ) THEN
    ALTER TABLE public.invoices
      ADD CONSTRAINT invoices_shipping_state_check
      CHECK (shipping_state IN ('not_required', 'ready_to_ship', 'shipped', 'completed', 'cancelled'));
  END IF;
END $$;
