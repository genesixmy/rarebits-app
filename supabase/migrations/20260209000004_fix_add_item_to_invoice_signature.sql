-- Fix ambiguous add_item_to_invoice overload (keep only the 5-arg signature with quantity + price)

-- Remove the old signature that causes PostgREST ambiguity
DROP FUNCTION IF EXISTS public.add_item_to_invoice(UUID, UUID, UUID, INTEGER, NUMERIC);

-- Recreate the intended signature with availability checks (reserved-aware)
