-- SHIP-2: Paid flow shipping pass-through
-- Wallet in should use invoice total that includes shipping_charged.
-- Revenue/profit reporting remains item-line based (invoice_items.line_total), so shipping is not treated as revenue.
...
DECLARE
  ...
  v_recalculated_subtotal NUMERIC;
  v_shipping_charged NUMERIC;
  v_total_with_shipping NUMERIC;
...
BEGIN
  ...
  INTO v_recalculated_subtotal
  FROM public.invoice_items ii
  WHERE ii.invoice_id = v_invoice.id;

  v_shipping_charged := GREATEST(COALESCE(v_invoice.shipping_charged, 0), 0);
  v_total_with_shipping := v_recalculated_subtotal + v_shipping_charged;

  UPDATE public.invoices
  SET
    subtotal = v_recalculated_subtotal,
    total_amount = v_total_with_shipping,
    shipping_charged = v_shipping_charged,
    updated_at = NOW()
  WHERE id = v_invoice.id;

  v_invoice.total_amount := v_total_with_shipping;
  v_invoice.shipping_charged := v_shipping_charged;
